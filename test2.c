...
/*   Updated: 2014/11/23 18:53:58 by hhismans         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	case_horiz(t_env env, t_point p1, t_point p2, int color)
{
	...
	while (xi <= p2.x)
	{
		mlx_pixel_put(env.mlx, env.win, xi, yi, color);
		e = e + coef;
		if (e >= 0)
		{
			yi++;
		...
		if (dx > 0)
		{
			...
			else
			{
				if (p1.x < p2.x)
				{
					while (p1.x != p2.x)
					{
						mlx_pixel_put(env.mlx, env.win, p1.x, p2.y, color);
						p1.x++;
					}
				}
				else
